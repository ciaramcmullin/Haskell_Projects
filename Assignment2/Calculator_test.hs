-- Ciara McMullin cbm389@nyu.edu
-- worked with Feliks Leus fl1095@nyu.edu
...
example :: Expr
example = times (minus (Lit 1) (Lit 2)) (Lit 3)
...
-- function that takes operator, binary opertor and two numbers x and y and evaluates expression
interp (Op Plus x y) = interp x + interp y
interp (Op Minus x y) = interp x - interp y
interp (Op Times x y) = interp x * interp y
...
-- example testing other numbers
--example2 :: Expr
--example2 = (plus (Lit 0) (Lit 3))

simplifyZero :: Expr -> Expr
-- BEGIN simplifyZero (DO NOT DELETE THIS LINE)
-- let e + 0 equal e to simplify expression
...
-- we compare the simplified version of interp to the interp to compare and see if we obtain the same result
...
prop_optimizer_idempotent e = (simplifier e == e2) 
                              where e2 = simplifier e
...
-- we use peephole before we check for findPlusZero to exvaulate sub expressions so all expressions in large or small are looked at
...
expr_shrink e@(Op b e1 e2) =
    [Lit (interp e), e1, e2] ++ [Op b e1' e2 | e1' <- expr_shrink e1] ++ [Op b e1 e2' | e2' <- expr_shrink e2]
...
peephole f (Lit x) = Lit x
peephole f (Op b x y) = f (Op b (peephole f x) (peephole f y))
...
run [] y = Just y
run (x:xs) y  --(y:z:ys) How to concat Just(stack)? hd has Maybe Stack type
  | step x y == Nothing = Nothing
  | otherwise = run xs hd
   where hd = unpack (step x y)

unpack :: Maybe Stack -> Stack
unpack (Just x) = x
unpack Nothing = []
...
--compile (Op x y z) = (IOp x):[] ++ (compile y) ++ (compile z)   <===== Original version
...
prop_compile_decompile e = decompile (helper_compile e) == Just e
...
prop_decompile_compile i
  | helper (decompile i) = helper_compile k == i
  | i == [] = True
  | otherwise = False
      where Just k = decompile i
-- END prop_decompile_compile (DO NOT DELETE THIS LINE)

helper :: Maybe Expr -> Bool
helper (Just e) = True
helper Nothing = False

helper_compile :: Expr -> [Instr]
-- BEGIN compile (DO NOT DELETE THIS LINE)
-- no points for cheekily emitting a single PUSH instruction
helper_compile (Lit a) = (IPush a):[]
--compile (Op x y z) = (IOp x):[] ++ (compile y) ++ (compile z)   <===== Original version
helper_compile (Op x y z) =  (helper_compile y) ++ (helper_compile z) ++ (IOp x):[]
