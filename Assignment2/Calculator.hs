...
expr_shrink (Lit e) = []
expr_shrink e@(Op b x y) = [Lit (interp e), x, y] ++ [Op b x2 y | x2 <- expr_shrink x] ++ [Op b x y2 | y2 <- expr_shrink y]
...
prop_compile_correctness :: Expr -> Bool   -- Is definition complete?
...
prop_decompile_compile i
  | helper (decompile i) = compile k == i
  | i == [] = True
  | otherwise = False
