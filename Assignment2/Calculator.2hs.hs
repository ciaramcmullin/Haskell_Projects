...
data Expr = Op BinOp Expr Expr -- Add BinOp before Expr Expr
          | Lit Int
    deriving (Show, Eq)
...
example :: Expr
example = Op Times (Op Plus (Lit 10) (Lit 2)) (Lit 3)
--example = (Op Plus (Lit 2) (Lit 0))
--example = Lit 9
...
interp (Lit a) = a
--interp (Op x y (Lit a)) = interpBinOp x interp y a
--interp (Op x (Lit a) z) = interpBinOp x a interp z
--interp (Op x (Lit a) (Lit b)) = interpBinOp x a b
--interp (Op x y z) = interpBinOp x interp y interp z

interp (Op Plus y z) = interp y + interp z
interp (Op Minus y z) = interp y - interp z
interp (Op Times y z) = interp y * interp z
...
prop_optimizer_correctness x = interp x == interp (simplifier x) -- what is it trying to accomplish?
...
prop_optimizer_idempotent x = simplifier (simplifier x) == simplifier x
...
prop_optimizer_optimizes (Lit _) = False
prop_optimizer_optimizes x = findPlusZero (simplifier x)
...
--peephole f (Op _ y _) = peephole f y
--peephole f (Op _ _ z) = peephole f z
--peephole f (Op _ _ z) = f z
...
--step (IPush y) [] = Nothing

--step (IOp o) (x:_) = Nothing
step (IPush x) xs =  Just (x:xs)
step (IOp o) (x:y:ys) = Just (((interpBinOp o) y x):ys)-- implement POP if it has more than two elements
    --where z = (interpBinOp o) y x
--step (IOp o) (x:y:[]) = Just (((interpBinOp o) y x):[])
    --where z = (interpBinOp o) y x
step (IOp o) (x:_) = Nothing
step (IOp o) [] = Nothing
    --where z = (interpBinOp o) y x
...
run ([]) y = Nothing
--run (x:y:xs) y = run xs hd  --(y:z:ys) How to concat Just(stack)? hd has Maybe Stack type
  --  where hd = step x y
...
compile (Op x y z) = (compile y) ++ (IOp x):[] ++ (compile z)
...
--prop_compile_correctness e = run (compile e) ([]) == (interp e:[])
